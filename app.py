        ...
        try:
            ...
            if data["resultCount"] > 0:
                ...
                return render_template(
                    ...
                    artwork=app_info.get(
                        "artworkUrl512",
                        "https://placehold.co/512x512/dcdcdc/000000?text=No+Image",
                    ),
                ...
                return render_template(
                    "apple_lookup.html",
                    error=f"No app found for Bundle ID: {bundle_id}",
                )
            ...
            return render_template(
                "apple_lookup.html",
                error=f"An error occurred while fetching data from Apple: {e}",
            )
        except ValueError:
            # Handle JSON decoding errors
            return render_template(
                "apple_lookup.html", error="Error parsing JSON response from Apple API."
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                ...
                artwork=app_info.get(
                    "icon", "https://placehold.co/512x512/dcdcdc/000000?text=No+Image"
                ),
            ...
            return render_template(
                "google_lookup.html",
                error=f"No app found for Package Name: {bundle_id}",
            )
        except Exception as e:
            # Catch any other general errors from the scraper
            return render_template(
                "google_lookup.html",
                error=f"An error occurred while fetching data from Google Play: {e}",
            )
